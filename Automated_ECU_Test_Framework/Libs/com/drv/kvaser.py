...
class KVASER():
    ...
    def __init__(self):
        ...
        try:
            ...
            print "INFO: Failed to load canlib32.dll, reasons could be:"
            print "  - 64 bits version of Python installed. It must be 32 bits version (also called x86)"
            print "  - KVASER drivers not installed. Please execute LATTE\Com\Trunk\drivers\kvaser\kvaser_drivers_w2k_xp.exe"
            self.canlib_loaded = False
            raise WindowsError("ERROR: Failed to load canlib32.dll")
    ...
    def scan_devices(self):
        ...
        if state < 0:
            raise Exception('INFO: KVASER device can not be accessed')
