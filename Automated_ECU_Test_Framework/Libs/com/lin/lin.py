...
1.6.0 [MPeriago] Implemented lin master node behavior.
      [CBlanco] IMPROVES from MQB and BCM added to Official LATTE library:
          ...
          Added 'disable_frame_response' method which disables the response of specific LIN message ex. when no response for SlaveResp is required
          ...
          Added 'get_slave_productid_by_name' & 'get_slave_productid_by_nad' methods which return 'supplier_id','function_id' and 'variant_id' extracted from LDF for a specific slave
          ...
          Added 'get_schedule_frames_publisher' method which returns list with publishers of frames supported by schedule passed
...
class LIN:
    ...
    def __init__(self, index, ldf, mode):
        ...
        '''
        #print '\nLIN channel open in channel ID ' + str(index) + '\n'
        self.index = index
        self.ldf = ldf
        # Init some vars
        ...
        while self._txrx_thread_active:
            if self.is_master == True:
                if self.ldf.scheduling_tables[self.active_table_index].ids != []: # There is something to be sent
                    ...
                    if elapsedTime >= LIN_TICK_TIME_SECONDS:
                        start_time = time.clock()
                        self.ldf.scheduling_tables[self.active_table_index].counter += LIN_TICK_TIME
                        if self.ldf.scheduling_tables[self.active_table_index].counter >= self.ldf.scheduling_tables[self.active_table_index].delay[self.sch_frame_index]:
                            self._send_request(self.ldf.scheduling_tables[self.active_table_index].ids[self.sch_frame_index])
    ...
    def get_plot_signal_frame(self, signalName, frameName = None):
        ...
        class Signal():
            offset = 0
            signalLength = 0
            littleEndianStart = False
            signalName = ''
            
        signal = Signal()
        signal.signalName = signalName
        linid, signal.offset, signal.signalLength = self.ldf.find_signal_info(signalName)
        if linid != []:
            for item in self._frames_received:
                if item.linid == linid:                                        
                    return (item.plotBuffer, signal, self.ldf, (self.plot_start_time, self.plot_stop_time)) # signal info to be used with plot class
    ...
    def _write_frame(self, linid, dlc, data, faulty_checksum=False):
        '''
        Description: Writes LIN frame to channel.
        '''
        self.write_lin_frame(self.index, linid, dlc, data, faulty_checksum)

    def _write_frame_master(self, linid, dlc, data, faulty_checksum=False):
        '''
        Description: Writes LIN master frame to channel.
        '''
        self.write_lin_frame_master(self.index, linid, dlc, data, faulty_checksum)
    ...
    def _send_request(self, linid):
        '''
        Description: Sends a lin master request in channel.
        
        '''
        self.send_lin_request(self.index, linid)
    ...
    def set_frames_response(self, element_list, enable):
        '''
        Description: Enables/disables the response of a list of frames or modules. These frames can be the name of the frame, 
        the frame identifier or a module name. It's also possible to pass one item, not a list. So param 'element_list' can be:
        ...
        for item in element_list:
            linid = None
            if (type(item) == str):
                linid = self.ldf._get_lin_id_from_frame_name(item)
    ...
    def set_checksum_response(self, element_list, faulty):
        '''
        Description: Sets/unsets a faulty checksum in the response of a list of frames or modules. These frames can be the name of the frame, 
        the frame identifier or a module name. It's also possible to pass one item, not a list. So param 'element_list' can be:
            - A module name, frame name or frame id.
            - A list of a combination of the above.
    ...
    def set_signal(self, signalName, value, frameName = None):
        '''
        Description: Sets value in signal. The frame with this signal will be sent when the master (ECU) puts the header in the bus.
        ...
        '''
        linid, dlc, frame = self.ldf.write_signal_to_frame(signalName, value, frameName)
    ...
    def get_signal(self, signalName):
        '''
        Description: Finds the signal in the frames buffer and return the signal value. The signal is usually a frame that the master (ECU)
        puts the header and data, or that other real slave as sent to the bus after a request from the master.
        ...
        '''
        linid, startbit, length = self.ldf.find_signal_info(signalName)
        if linid != []:
    ...
    def get_frame(self, frameId):
        ...
        Example:
            ...
            lin_id, startbit, length = lin1.ldf.find_signal_info('SignalName')
    ...
    def report_frame_info(self, frameId):
        '''
        Description: Returns frame information
        Returns [name, dlc]
        '''
        return self.ldf.find_frame_info(frameId)
