# Embedded file name: s:\TOOLS_DEV\LATTE\Com\Trunk\code\com\fry\fry.py
import threading
...
class FLEXRAY(object):
    ...
    def _rx_task(self):
        ...
        while self._rx_thread_active:
            fr, fr_status = self.read_fr_frame(self.index)
    ...
    def _add_frame_to_rcv(self, fr, base, frame_name):
        ...
        frRcv.base = base
        frRcv.slotid, frRcv.event_type, frRcv.cycle_count, frRcv.payload_length, frRcv.data = fr
    ...
    def _transmit_frame(self, frame, mode, data = []):
        ...
        for fr_id in tx_frame.fr_id:
            self.write_fr_frame(self.index, fr_id, mode, tx_frame.data, tx_frame.payload_length, tx_frame.repetiton, tx_frame.offset, 1)
    ...
    def stop_tx_frames(self):
        ...
        Example:
            ...
            fr_1.stop_tx_frames()
        """
        tx_frames = list(self._frames_transmitting)
