# Embedded file name: s:\TOOLS_DEV\LATTE\Com\Trunk\code\com\fry\fibex.py
import sys
from ctypes import *
...
class ClusterConfig(object):
    ...
    def __init__(self):
        ...
        self.pKeySlotUsedForStartup = 1
        self.pKeySlotUsedForSync = 1
        self.pLatestTx = 0
...
class _Coding(object):
    ...
    def __init__(self):
        ...
        self.computation_method = 'Not Defined'
        self.compu_numerator = [0] * 2
...
class FIBEX(object):
    ...
    def __init__(self, fibex_file, fr_channel, cold_start_slots):
        ...
        self.hTableOfFrames = {}
        self.hTableOfPDUs = {}
        self.hTableOfSignals = {}
        self.hTableOfSignals3 = {}
        self.hTableOfOffsets = {}
        self.hTableOfSwitchPDUs = {}
    ...
    def _set_controller_config(self, fr_channel, cold_start_slots):
        ...
        self.cluster_config.erayChannel = fr_channel
    ...
    def _fill_frames_table(self):
        ...
        for channel in channels:
            ...
            channel_in_used = channel.find('{http://www.asam.net/xml}FLEXRAY-CHANNEL-NAME')
    ...
    def _fill_offsets(self):
        ...
        for frame in self.hTableOfFrames:
            for slot_id in self.hTableOfFrames.get(frame).slot_id:
                ...
                self.hTableOfOffsets[slot_id][self.hTableOfFrames.get(frame).offset] = [self.hTableOfFrames.get(frame).repetition, frame]
    ...
    def _fill_pdus_table(self):
        ...
        for pdu in pdus:
            ...
            if found_pdu is None:
                self.hTableOfSwitchPDUs[currentpdu.pdu_id] = currentpdu
            else:
        ...
        for pdu in pdus:
            ...
            if signal_instances is not None:
                for signal in signal_instances:
                    ...
                    if currentsignal.endianness == 'true':
        ...
        for coding in codings:
            ...
            if currentsignal.coding.computation_method == 'LINEAR':
                ...
                try:
                    ...
                    currentsignal.coding.compu_numerator[0] = 0
                    currentsignal.coding.compu_numerator[1] = 1
    ...
    def look_for_frame_from_pdu(self, pduName):
        ...
        for frame in self.hTableOfPDUs:
            ...
            for pdu in pdus:
                if pdu.pdu_name.lower() == pduName.lower():
    ...
    def prepare_frame_to_tx(self, frame, data):
        ...
        if data == []:
            if signals[0].endianness == 0:
                for signal in signals:
                    payload |= signal.value << signal.bitpos

                for i in range(frame.length):
                    data.append(payload >> i * 8 & byte_mask)
                ...
                data = deque()
                for i in range(frame.length):
                    data.appendleft(payload >> i * 8 & byte_mask)

                data = list(data)
        tx_frame.payload_length = frame.length / 2 + frame.length % 2
    ...
    def read_value_in_frame(self, byte_pos, bit_pos, bit_length, data, endianness = 0):
        ...
        """
        values = []
        if 8 - bit_pos % 8 < bit_length:
            values.append(data[byte_pos] >> bit_pos % 8)
            mask2 = 2 ** (bit_length - 8 + bit_pos % 8) - 1
            if mask2 < 255:
                if endianness == 0:
    ...
    def raw_to_phys(self, signalName, raw_value):
        ...
        if localsignal.coding.computation_method == 'LINEAR':
            phys_val = (localsignal.coding.compu_numerator[1] * raw_value + localsignal.coding.compu_numerator[0]) / localsignal.coding.compu_denominator
    ...
    def burst_frame_to_tx(self, frame, rawdata, size):
        ...
        tx_frame = _FrameToTx()
        data = rawdata
        tx_frame.payload_length = size / 2 + size % 2
        tx_frame.repetiton = frame.repetition
        tx_frame.offset = frame.offset
        tx_frame.data = data
        tx_frame.fr_id = frame.slot_id
