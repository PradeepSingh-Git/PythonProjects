...
CHANGE LOG
==========
1.2.11 Bugfix: Removed unnecessary return value in send_request method.
       Bugfix: Using copy of list in _send_request_one method.
...
class ISO15765_2():
    ...
    def _write_frame(self, data):
        ...
        time_elapsed = 0.0
        self.write_dgn_frame(self._req_canid, len(data), data)
        fr = self.read_dgn_frame(self._req_canid)
    ...
    def _read_frame(self):
        ...
        fr = self.read_dgn_frame(temp_id)
        while fr == [] and time_elapsed < self._rx_timer:
            time_elapsed = time.clock() - time_start
            fr = self.read_dgn_frame(temp_id)
        if fr != []:
